...
do
	echo $gene
        grep ">" ../output/"$gene"/pangraph/"$gene"_extracted.fa  | tr -d '>'  | cut -d ' ' -f 1 > curves/"$gene"_accs_all.txt
	grep "0 diffs" ../output/"$gene"/pangraph/"$gene"_extracted.fa  | tr -d '>'  | cut -d ' ' -f 1 > curves/"$gene"_accs_focal_gene.txt
	...
	python ../../scripts/positional_entropy.py --json ../output/"$gene"/pangraph/"$gene"_pangraph.json --normalise --consensus --subset curves/"$gene"_accs_focal_gene.txt > entropies/"$gene"_entropies_focal_gene.txt
	python ../../scripts/positional_entropy.py --json ../output/"$gene"/pangraph/"$gene"_pangraph.json --normalise --consensus --subset curves/"$gene"_accs_all.txt > entropies/"$gene"_entropies_all.txt
        python ../../scripts/positional_entropy.py --json ../output/"$gene"/pangraph/"$gene"_pangraph.json --normalise --consensus --subset curves/"$gene"_accs_focal_gene_deduplicated.txt >    entropies/"$gene"_entropies_focal_gene_deduplicated.txt
	python ../../scripts/positional_entropy.py --json ../output/"$gene"/pangraph/"$gene"_pangraph.json --normalise --consensus --subset curves/"$gene"_accs_all_deduplicated.txt > entropies/"$gene"_entropies_all_deduplicated.txt
	# plot the transposase density
	python plot_transposase_density.py --dist_csv ../output/"$gene"/pangraph/"$gene".output_dists.csv --gff_file /Users/Liam/Downloads/test-bl/input/gffs/"$gene"_annotations.gff --gene_name "$gene" --output_html curves/"$gene"-curves-focal-gene.html --flanking_width 5000 --entropies entropies/"$gene"_entropies_focal_gene.txt --strain_list curves/"$gene"_accs_focal_gene.txt
